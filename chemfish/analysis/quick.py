...
from chemfish.factories.caches.stim_cache import *
from chemfish.factories.caches.video_cache import *
...
class Quick:
    ...
    auto_fix: bool = True
    discard_controls: Set[ControlLike] = DEFAULT_TRASH_CONTROLS
    ...
    def query_runs(self, wheres: Union[RunsLike, ExpressionsLike]) -> List[Runs]:
        ...
        try:
            ...
            wheres = Runs.id << {r.id for r in Runs.fetch_all(wheres)}
        except Exception:
            pass
        query = (
            Runs.select(
    ...
    def _no_such_features_message(self, run) -> str:
        ...
        if isinstance(run, (str, int, float, Runs, Submissions)):
            ...
            feats_defined = (
                ...
                + "; ".join(
                    [
                        "r" + str(ValarRuns.fetch(r)) + ": " + ", ".join(ValarTools.features_on(r))
                        for r in run
                    ]
                )
                + "]"
            ...
            return "The feature {} is not defined on runs {}. These are: {}".format(
                self.feature,
                ",".join(["r" + str(r.id) for r in ValarRuns.fetch_all(run)]),
                feats_defined,
            )
