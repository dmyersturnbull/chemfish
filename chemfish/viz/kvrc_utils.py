...
class Key:
    ...
    def resolved(self, value: Optional[str]) -> Key:
        ...
        :param value: Should be None iff it was not set in config file
        :return: A new Key
...
class KvrcConfig:
    ...
    def key(
        ...
        :param key: The name of the key (ex: 'stamp_font_size')
        :param kind: A function mapping a user-passed string to the required type
        :param fallback: A default value of type T applied when the key isn't listed in the config file
        :param nullable: Whether the user can set the key to None (ex: 'stamp_font_size=none'); independent of the fallback
        :param d: An optional description of the key
        :return: The resolved value of the inserted key; this is the same as `config.collection['stamp_font_size'].value)
        """
...
class KvrcCore:
    ...
    def print_wrapped(self, length: int = 75) -> None:
        ...
        :param length: Max number of characters per line
        """
        print(
            "\n".join(
                [
